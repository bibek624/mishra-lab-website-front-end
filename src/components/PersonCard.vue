...
<template>
  <router-link :to="{ name: 'people-profile', params: { slug: person.slug } }" class="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4 flex flex-col items-center text-center w-[320px] mx-auto">
    <img :src="person.photo" :alt="person.name" class="h-20 w-20 md:h-24 md:w-24 rounded-full object-cover" />
